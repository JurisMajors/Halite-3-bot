...
class StateMachine():
    ...
    def enough_backup_nearby(self, pos, distance, amount):
        ...
        for my_ship in self.me.get_ships():
            distance_to_pos = self.game_map.euclidean_distance(
    ...
    def exists_better_in_area(self, cntr, current, area):
        ...
        current_factor = self.game_map.cell_factor(
            cntr, self.game_map[current], self.me, self.NR_OF_PLAYERS)
        for y in range(area):
            for x in range(area):
                ...
                if cell.halite_amount >= self.game_map.HALITE_STOP:
                    other_factor = self.game_map.cell_factor(
                        cntr, cell, self.me, self.ENABLE_BACKUP)
    ...
    def collecting_transition(self):
        ...
        if self.ship.is_full:
            ...
            # Keep exploring if halite patch low on halite and ship doesnt have
            # close to returning percentage
            self.DP.process_new_destination(self.ship)
            return "exploring"
            ...
            # if not that mch halite and not too close to assasinate and enemy
    ...
    def returning_transition(self):
        if self.game.turn_number >= GC.CRASH_TURN and self.game_map[self.ship.position].parent.is_occupied\
            ...
            self.ship_dest[self.ship.id] = self.game_map[
                self.ship.position].parent.position
            return "assassinate"
    ...
    def builder_transition(self):
        ...
        distance_to_dest = self.game_map.euclidean_distance(
            self.ship.position, self.ship_dest[self.ship.id])

        if future_dropoff_cell.has_structure:
            if distance_to_dest <= GC.CLOSE_TO_SHIPYARD * self.game_map.width:
                self.ship_dest[self.ship.id] = self.GF.bfs_unoccupied(
                    future_dropoff_cell.position)
                self.DP.reassign_duplicate_dests(
                    self.ship_dest[self.ship.id], self.ship.id)
            else:
                self.DP.process_new_destination(self.ship)
            ...
            smallest_dist = self.GF.dist_to_enemy_doff(
                self.ship_dest[self.ship.id])
    ...
    def get_best_neighbour(self, position):
        ...
        max_halite = current.halite_amount * \
            self.game_map.get_inspire_multiplier(
                position, current, self.ENABLE_BACKUP)
        best = current
        for n in neighbours:
