...
class GF():
    ...
    def process_new_destination(ship):
        ...
        ship_h = GF.halite_priority_q(source, SHIP_SCAN_AREA)
    ...
    def exploring_transition(ship):
        ...
        elif GF.amount_of_enemies(ship.position, 4) >= 2:
            # for inspiring
            ship_dest[ship.id] = GF.get_best_neighbour(ship.position).position

        elif euclid_to_dest <= 5 and GF.exists_better_in_area(ship.position, ship_dest[ship.id], 4):
            ship_h = GF.halite_priority_q(
    ...
    def returning_transition(ship):
        new_state = None
        if ship.position in GF.get_dropoff_positions():
    ...
    def fleet_transition(ship):
        ...
        if ship.position == destination:  # if arrived
            ...
            if game_map[destination].is_occupied:
                ship_dest[ship.id] = GF.get_best_neighbour(destination).position
    ...
    def builder_transition(ship):
        ...
        if future_dropoff_cell.has_structure:
            if distance_to_dest <= CLOSE_TO_SHIPYARD * game_map.width:
                ship_dest[ship.id] = GF.bfs_unoccupied(future_dropoff_cell.position)
            else:
                GF.process_new_destination(ship)
                new_state = "exploring"
            ...
            smallest_dist = GF.dist_to_enemy_doff(ship_dest[ship.id])
    ...
    def state_transition(ship):
        ...
        if game.turn_number >= CRASH_TURN and game_map.calculate_distance(
        ...
        elif ship.position in GF.get_dropoff_positions():
            ship_path[ship.id] = []
            GF.find_new_destination(
                game_map.halite_priority, ship)
            new_state = "exploring"
            ...
            new_state = GF.exploring_transition(ship)

        elif ship_state[ship.id] == "collecting":
            new_state = GF.collecting_transition(ship)

        elif ship_state[ship.id] == "returning":
            new_state = GF.returning_transition(ship)

        elif ship_state[ship.id] == "fleet":
            new_state = GF.fleet_transition(ship)

        elif ship_state[ship.id] == "build":
            new_state = GF.builder_transition(ship)

        elif ship_state[ship.id] == "waiting":
            new_state = GF.waiting_transition(ship)

        elif ship_state[ship.id] == "backup":
            new_state = GF.backup_transition(ship)

        if new_state is not None:
            GF.state_switch(ship.id, new_state)
    ...
    def should_build():
        ...
        return clusters_determined and len(me.get_ships()) > (len(me.get_dropoffs()) + 1) * FLEET_SIZE and cluster_centers \
               and GF.fleet_availability() >= 1.5 * FLEET_SIZE and not GF.any_builders()
    ...
    def send_ships(pos, ship_amount, new_state, condition=is_fleet, leader=None):
        ...
        h = GF.halite_priority_q(pos, SHIP_SCAN_AREA)
    ...
    def clusters_with_classifier():
        ...
        # do filtering
        cluster_centers = GF.filter_clusters(cluster_centers, MAX_CLUSTERS)
    ...
    def filter_clusters(centers, max_centers):
        ...
        for i, d in enumerate(centers_copy, start=0):
            ...
            if diff < CLOSE_TO_SHIPYARD * game_map.width or GF.dist_to_enemy_doff(pos) < CLOSE_TO_SHIPYARD * game_map.width:
                if d in centers:
                    centers.remove(d)
                continue
    ...
    def process_building(cluster_centers):
        ...
        if fleet:
            ...
            if game_map.width >= EXTRA_FLEET_MAP_SIZE:
                GF.send_ships(dropoff_pos, int(FLEET_SIZE / 2), "fleet", leader=closest_ship)
...
while True:
    ...
    if game.turn_number == CRASH_SELECTION_TURN:
        CRASH_TURN = GF.select_crash_turn()
    ...
    if not dropoff_built and 2.5 * (GF.max_enemy_ships() + 1) > len(me.get_ships()) and game.turn_number <= SPAWN_TURN \
