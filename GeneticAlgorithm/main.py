...
def selection():
	...
	sorted_by_fitness = sorted(fitness.items(), key=lambda k: k[1], reverse = True)[:PARENT_AMOUNT]
...
def crossover(selected_versions):
	...
	couples = [(selected_versions[v1], selected_versions[v2]) for v1 in range(len(selected_versions)) for v2 in range(v1+1,len(selected_versions))]
	# need to randomly remove len(couples) - (POPULATION_SIZE - PARENT_AMOUNT) couples
	...
	for couple in couples: # add new versions
		biggest_version += 1
		population[biggest_version] = [biggest_version] + create_off_spring(population[couple[0]], population[couple[1]])
