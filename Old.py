...
SAVIOR_FLEET_SIZE = 3 if NR_OF_PLAYERS == 2 else 2
ENABLE_COMBAT = False
...
def state_transition(ship):
    ...
    elif ship_state[ship.id] not in ["fleet", "build", "waiting"] and ship.halite_amount <= constants.MAX_HALITE * (return_percentage * 0.7) and better_patch_neighbouring(ship, 250):
...
while True:
    ...
    if len(crashed_ships) > 0 and not game.turn_number >= CRASH_TURN and ENABLE_COMBAT:
        ...
        if len(crashed_positions) > 0:
            ...
            if (amount_of_enemies(crashed_pos, 6) >= 2 or hal >= 1000) and not have_less_ships(0.8):
                send_ships(crashed_pos, SAVIOR_FLEET_SIZE, is_savior)
